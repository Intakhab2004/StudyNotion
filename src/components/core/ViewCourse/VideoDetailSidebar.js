...
const VideoDetailSidebar = ({setReviewModal}) => {
    ...
    const {courseEntireData, completedLectures, totalNoOfLectures, courseSectionData} = useSelector((state) => state.viewCourse);
                ...
                <div className={`${showSidebar ? "hidden" : ""} transition-all origin-right duration-500 flex h-[calc(100vh-3.5rem)] w-[320px] max-w-[350px] flex-col border-r-[1px] border-r-richblack-700 bg-richblack-800 offSidebar2`}>
                    <div className={`${showSidebar ? "hidden" : ""} mx-5   flex flex-col items-start justify-between gap-2 gap-y-4 border-b border-richblack-600 py-5 text-lg font-bold text-richblack-25 offSidebar2`}>
                        <div className='flex w-full items-center justify-between '>
                            ...
                            <IconBtn text={"Review"} onClickHandler={() => { setReviewModal(true) }} />
                        </div>
                                ...
                                <details key={index} className=' appearance-none text-richblack-5 detailanimatation'>
...

                                    {
                                        section?.subSection.map((lecture, index) => (
                                            <div className='transition-[height] duration-500 ease-in-out' key={lecture._id}>
                                                <div
                                                   onclick = {() => {if(window.innerWidth < 1024){
                                                            setShowSidebar(true)
                                                        }
                                                        navigate(`/view-course/${courseId}/section/${section?._id}/sub-section/${lecture?._id}`);
                                                    }} className={`${lecture?._id === videoActive ? ("bg-yellow-200") : ("bg-richblack-50")} cursor-pointer items-baseline  flex gap-3  px-5 py-2 font-semibold text-richblack-800 relative border-b-[1px] border-richblack-600 `}
                                                >
                                                   ...
                                                   <p className=' ml-6'>{lecture?.title}</p>
                                                </div>
                                            </div>
                                        ))
                                    }
