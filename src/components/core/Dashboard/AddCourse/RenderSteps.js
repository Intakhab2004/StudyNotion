        ...
        <div>
                    ...
                    steps.map((item) => (
                        <>
...

                            {
                                ...
                                <>
                                    <div className={`h-[calc(34px/2)] w-[33%] border-b-2 border-dashed ${item.id < step ? "border-yellow-50" : "border-richblack-500"}`}></div>
                                </>
                            }
                        </>
                    ))
            ...
            <div className='mb-10 md:mb-16'>
                <div className='hidden md:flex justify-between select-none '>
                    {
                        steps.map((item) => (
                            <div key={item.id} className={`min-w-[130px] text-center text-sm ${item.id <= step ? "text-richblack-5" : "text-richblack-500"}`}>
                                {item.title}
                            </div>
                        ))
                    }
