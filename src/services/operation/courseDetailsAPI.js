...
const {  GET_INSTRUCTOR_COURSE_API,
         DELETE_COURSE_API,
...
export const fetchInstructorCourse = async(token) => {
    ...
    try{
        const response = await apiConnector("GET", GET_INSTRUCTOR_COURSE_API, null, {Authorization: `Bearer ${token}`});
        console.log("INSTRUCTOR COURSE API RESPONSE IS.....", response);
...
export const createSection = async(data, token) => {
    ...
    try{
        ...
        result = response?.data?.currentCourse
...
export const getFullCourseDetails = async(courseId, token) => {
    ...
    try{
        const response = await apiConnector("POST", GET_FULL_COURSE_DETAILS_API, {courseId}, {Authorization: `Bearer ${token}`});
